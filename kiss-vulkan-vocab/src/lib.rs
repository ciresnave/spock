...
impl CoopMatrix {
    ...
    /// The canonical enumeration string: the bytes the threshold measures and
    /// the digest hashes.
    ///
    ...
    /// implementations of one rule; the first draft of this manifest did
    /// exactly that and got the cooperative-VECTOR tuple wrong, which would
    /// have published a `digest_input` nobody could reproduce.
    pub fn canonical_enumeration(&self) -> Option<String> {
        match self {
            Self::None | Self::Digest(_) => None,
            Self::Shapes(shapes) => Some(
                shapes
                    .iter()
                    .map(CoopShape::spell)
                    .collect::<Vec<_>>()
        ...
        match self {
            ...
            Self::Shapes(_) => {
                let joined = self
                    ...
                    .expect("Shapes always has an enumeration");
...
impl CoopVector {
    ...
    /// The canonical enumeration string — see
    /// [`CoopMatrix::canonical_enumeration`], which this mirrors for the
    /// cooperative-vector field.
    ///
    /// The two fields measure and digest **independently**, on their own bytes,
    /// so they need their own accessor rather than a shared one.
    pub fn canonical_enumeration(&self) -> Option<String> {
        match self {
            Self::None | Self::Digest(_) => None,
            Self::Combos(combos) => Some(
                combos
                    .iter()
    ...
    fn spell(&self) -> String {
        match self {
            ...
            Self::Combos(_) => {
                let joined = self
                    .canonical_enumeration()
                    .expect("Combos always has an enumeration");
                if joined.len() <= COOP_DIGEST_THRESHOLD {
                    format!("cv-{joined}")
                } else {
