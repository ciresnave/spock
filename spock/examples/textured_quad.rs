...
//!
//! Run with: `cargo run -p spock --features fetch-spec --example textured_quad`
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let spv_bytes = std::fs::read(&spv_path).map_err(|e| {
        format!(
            "could not read {spv_path}: {e} \
             (run `cargo run -p spock --features naga,fetch-spec --example compile_shader`)"
        )
    })?;
    ...
    let instance = match Instance::new(InstanceCreateInfo {
        application_name: Some("spock textured_quad"),
